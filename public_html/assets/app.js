...
document.addEventListener('click',async event=>{
 ...
 try{
  ...
  if(action==='toggle-students'){const form=button.closest('form'),boxes=$$('input[name="student_ids"]',form),select=!boxes.every(box=>box.checked);boxes.forEach(box=>box.checked=select);$('[data-selected-count]',form).textContent=select?`Выбрано учеников: ${boxes.length}`:'Выбери одного или нескольких учеников';button.textContent=select?'Снять выбор':'Выбрать всех';}
