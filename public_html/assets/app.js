...
function actions(label){return `${errorBox()}<div class="modal-actions"><button type="button" class="btn secondary" data-action="close">Отмена</button><button class="btn" type="submit">${label}</button></div>`;}
...
document.addEventListener('submit',async event=>{
 ...
 try{
  ...
  else if(action==='save_task'){await refresh();detail(Number(result.id));toast('Задание сохранено. Теперь можно прикрепить файлы.');}
  else{closeModal();await refresh();toast(action==='submit'?'Решение отправлено на проверку':action==='change_password'?'Пароль изменён':'Изменения сохранены');}
